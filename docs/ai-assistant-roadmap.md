...
### 1. PDF 原文兜底索引 🥇 **最推荐先做**
- **问题**:`indexer.ts` 的 `listMarkdownItems()` 只索引带 `markdown` 附件(MinerU 转换过)的条目。刚导入、未转换的 PDF **AI 助手完全看不到**。而 MinerU 慢、耗 token、易失败。
- **目标**:导入即可问。
- **做法**:用已装的 `pdf-parse-new` 抽取 PDF 纯文本作为 fallback 建 FTS/向量索引;MinerU 转换完成后再"升级"为高质量 Markdown 版本。
- **工作量**:中。
...
1. **#1 PDF 原文兜底索引** — 覆盖面从"转过的"变成"全部",体验质变
2. **#2 重排序** — 改动小、见效快、+10~40% 准确率
3. **#3 引用跳转到 PDF 页** — 从"能引用"到"可核查"

三项均建立在现有架构上,无需重写。每项动工前走一遍 brainstorming → 方案设计。
